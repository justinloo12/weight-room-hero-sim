...
def add_pitcher_vulnerability_features(pitcher_df: pd.DataFrame, all_pitches_df: pd.DataFrame, batted_df: pd.DataFrame, pa_df: pd.DataFrame, use_season_stats: bool = True) -> pd.DataFrame:
    ...
    # NOTE: disabled when use_season_stats=False (e.g. leakage-safe holdout
    # evaluation — season aggregates would include future games).
    if use_season_stats and pitching_stats is not None:
        season_frames = []
...
def prepare_pitch_data(df):
    """Add derived flags to a raw Statcast pitch-level frame.

    Returns (all_pitches, batted). `batted` is the subset with tracked
    launch speed/angle; both frames keep the original index alignment.
    """
    df = df.copy()
    ...
    return all_pitches, batted


def build_profiles(all_pitches, batted, use_season_stats=True, verbose=True):
    """Aggregate hitter / pitcher / matchup profile tables from prepared
    pitch data. Everything is computed from the data passed in, so calling
    this on a date-truncated frame yields honest "as-of" profiles.

    Returns (hitter, pitcher, matchup, pa_df).
    """
    pa_df = build_pa_df(all_pitches)

    latest_game_date = all_pitches["game_date"].max()
    season_2026_mask = batted["game_date"] >= pd.Timestamp("2026-01-01")
    recent_2026_start = latest_game_date - pd.Timedelta(days=45) if pd.notna(latest_game_date) else pd.Timestamp("2026-01-01")
    recent_2026_mask = season_2026_mask & (batted["game_date"] >= recent_2026_start)

    batted_2026 = batted[season_2026_mask].copy()
    batted_recent = batted[recent_2026_mask].copy()
    ...
    if FEATURE_TOGGLES["pitcher_vulnerability"]:
        pitcher = add_pitcher_vulnerability_features(pitcher, all_pitches, batted, pa_df, use_season_stats=use_season_stats)
    ...
    return hitter, pitcher, matchup, pa_df
...
def assemble_matchup_frame(rows, hitter, pitcher, matchup, team_categories=None):
    """Merge profile tables onto matchup rows and derive every model feature.

    `rows` needs at minimum: batter, player_name (pitcher), stand, p_throws,
    home_team. It can be batted-ball events (training) or hypothetical
    batter-vs-starter rows (scoring). `team_categories`, if given, pins the
    home_team -> ballpark_code mapping so train and test encode identically.
    """
    final = rows.merge(hitter,  on="batter",                  how="left")
    ...
    if team_categories is not None:
        ballpark_code = pd.Categorical(final["home_team"], categories=team_categories).codes
    else:
        ballpark_code = final["home_team"].astype("category").cat.codes

    extra_cols = {
        "ballpark_code": ballpark_code,
...
def build_features(df, use_season_stats=True, team_categories=None, verbose=True):
    """Full feature build on a raw Statcast frame.

    Returns (final, hitter, pitcher, matchup): the batted-ball feature frame
    (without weather) plus the profile tables. Passing a date-truncated
    frame produces honest as-of features for that cutoff.
    """
    all_pitches, batted = prepare_pitch_data(df)
    hitter, pitcher, matchup, _ = build_profiles(
        all_pitches, batted, use_season_stats=use_season_stats, verbose=verbose
    )
    if verbose:
        print("Merging features...")
    final = assemble_matchup_frame(batted, hitter, pitcher, matchup, team_categories=team_categories)
    return final, hitter, pitcher, matchup
...
def main():
    ...
    final, hitter, pitcher, _ = build_features(df)
